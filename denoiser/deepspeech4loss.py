...
class DeepSpeech4Loss(PyTorchDeepSpeech):
    ...
    def compute_ppg(self,
        ...
        # Call to DeepSpeech model for prediction
        # result = self.DP_model(inputs.to(self._device), input_sizes.to(self._device))
        # print(f"Type: {type(result)}, Length: {len(result)}")  # 如果是 tuple
        outputs, output_sizes, _ = self.DP_model(inputs.to(self._device), input_sizes.to(self._device))
