...
class Solver(object):
    def __init__(self, data, model, optimizer, args):
        ...
        #                             device=self.device)
        print(os.getcwd())
        self.model_clap = laion_clap.CLAP_Module(enable_fusion=False, amodel='HTSAT-base')
        self.model_clap.load_ckpt('../../../../Weight/laion_clap/music_speech_audioset_epoch_15_esc_89.98.pt')
        self.model_clap.to(self.device)
        self.model_clap.eval()
    ...
    def _reset(self):
        ...
        # 提取语义 embedding（每个样本对应一个 embedding）

    def compute_clap_semantic_loss(self, clean_audio, denoised_audio, sample_rate=16000):
        clean_audio = clean_audio.squeeze(1)  # [B, L]
        denoised_audio = denoised_audio.squeeze(1)

        # 获取音频语义 embedding
        clean_emb = self.model_clap.get_audio_embedding_from_data(x=clean_audio, use_tensor=True)
        denoised_emb = self.model_clap.get_audio_embedding_from_data(x=denoised_audio, use_tensor=True)

        # 计算余弦相似度
        cos_sim = F.cosine_similarity(clean_emb, denoised_emb, dim=-1)  # [B]
        semantic_loss = 1.0 - cos_sim

        return semantic_loss.mean()  # 返回 batch 的平均语义损失
        ...
        for epoch in range(len(self.history), self.epochs):
            ...
            if self.tt_loader:
                ...
                with swap_state(self.model, self.best_state):
                    pesq, stoi, mcd, snr, psnr = evaluate(self.args, self.model, self.tt_loader)

                metrics.update({'pesq': pesq, 'stoi': stoi, 'mcd': mcd, 'snr': snr, 'psnr': psnr})
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            ...
            with torch.autograd.set_detect_anomaly(True):
                ...
                # print(clean.shape)
                # embed_clean = self.extract_semantic_embeddings(clean)
                # embed_denoised = self.extract_semantic_embeddings(estimate)

                # loss += self.compute_clap_semantic_loss(clean, estimate)
